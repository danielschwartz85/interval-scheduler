...
class Scheduler extends EventEmitter {
    ...
    /*
     * assign task and resolve with true or update existing one and resolve with false
     */
    assignTask(task) {
        let taskData = {
            id: task.id,
    ...
    /*
     * acquire and resolve with lock (auto released after 24 hours) should be unlocked
     */
    tryLock(lockBy) {
    ...
    /*
     * resolve with task serialized data
     */
    isScheduled(taskId) {
    ...
    queueSize() {
        return this._storage.queueSize();
    }

    taskSize() {
        return this._storage.taskSize();
    }
