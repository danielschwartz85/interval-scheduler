...
class Redis extends EventEmitter {
    ...
    taskSize() {
        ...
        let size = (cursor) => {
            let scan = this._client.scanAsync(cursor, 'MATCH', this._taskName('*'), 'COUNT', scanBatchSize)
                .then(([itr, tasks]) => {
                    let canceledData = tasks.map(t => this._isCanceled(t));
                    return Promise.all([itr, tasks, ...canceledData]);
                }).then(([itr, tasks, ...canceledData]) => {
                    count += tasks.filter((t, i) => !canceledData[i]).length;
                    return itr !== '0' ? size(itr) : Promise.resolve(count);
                });
            return scan;
        };
        return size(0);
    ...
    _isCanceled(taskIdKey) {
        return this._client.hgetAsync(taskIdKey, 'canceled').then(c => c === 'true');
    }
