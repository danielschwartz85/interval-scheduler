...
class Redis extends EventEmitter {
    ...
    constructor(config, taskIntervalSeconds = 60) {
        Common.assertKeys(config, 'port', 'host', 'db');
        super();
        this.config = JSON.parse(JSON.stringify(config));
        this.name = `${config['host']}:${config['port']}`;
        this.logger = Logger.tagged(['redis', this.name]);
        this._settings = {
            scheduleLock : {
                randLength : 12,
                autoReleaseSeconds : 24 * 60
            },
            indexQueueName : 'mainIndex',
            scriptNames : ['unlock', 'existsupdate', 'existsdisable', 'pulltask'],
            minTaskIntervalSeconds : Common.isLocal() ? 1 : 10,
            maxTaskIntervalSeconds : 60 * 60,
            scanBatchSize : 500
        };
        this._taskIntervalSeconds = taskIntervalSeconds;
        this._validate();
    }
    ...
    settings(path) {
        return Common.objGet(this._settings, path);
    }
    ...
    isScheduledCancel(taskId) {
        return this._lua('EXISTSDISABLE', this._taskName(taskId)).then(exists => {
    ...
    isScheduledUpdate(taskId, data, intervalSeconds = undefined) {
        ...
        return this._lua('EXISTSUPDATE', this._taskName(taskId), data, interval).then(exists => {
            return Promise.resolve(exists === 1);
        });
    }
    ...
    clearAllTasks() {
        ...
        let clearKeys = (cursor) => {
            this.logger.info(`searching tasks to delete cursor ${cursor}`);
